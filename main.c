        ...
        char** argv)
{
    ...
    const char* topic_xml = "<dds>"
            "<topic>"
            "<name>rt/NumTopic</name>"
            "<dataType>std_msgs::dds_::Int32_</dataType>"
            "</topic>"
            "</dds>";
    ...
    while (connected && count < max_topics)
    {
        ...
        ucdrBuffer ub;
        uint32_t topic_size = ros2Int_size_of_topic(&topic, 0);
        uxr_prepare_output_stream(&session, reliable_out, datawriter_id, &ub, topic_size);
        //ros2Int_serialize_topic(&ub, &topic);

        _Int32_cdr_serialize(&topic,&ub);

        printf("Send topic: id: %i\n", topic.data);
